...
К сожалению, запуск с платным vpn не был аппробирован. Запуск с бесплатным vpn - описан ниже.

==== antizapret
Ни для кого не секрет, что проект антизапрета использует openvpn и выбор достаточно богат.
Однако https://github.com/dperson/openvpn-clien[популярные сбоки] нуджно тюнить, из-за их киллсвитч режима (проксируй или умри):
Сервер антизапрета определяет, какие домены заблокированы и отклоняет такие соединения.
А они могут быть использованы для служебного использования в healthchecker, например у https://ntc.party/t/flaresolverr-vpn/3175[Flaresolverr]. Пока корректной докер сборки - нет.
